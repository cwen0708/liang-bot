...
持倉資訊中可能包含 `entry_horizon`（入場時的持倉週期）和 `entry_reasoning`（入場時的推理摘要）。
...
系統會根據你的 confidence 決定是否執行：
- **有策略支持**：confidence >= 配置門檻（通常 0.6）即可執行
- **無策略支持（覆蓋）**：confidence **必須 >= 0.7** 才會執行（倉位自動縮半）。低於 0.7 會被直接攔截為 HOLD
- confidence < 0.3：一律視為 HOLD

因此，若你要覆蓋策略做出 BUY 決策，confidence 必須給到 0.7 以上，否則等於白做。
注意：SELL（平倉）不受此限制，平倉是降低風險的動作，系統不會攔截。
...
因此，若你要覆蓋策略做出 BUY/SHORT 決策，confidence 必須給到 0.7 以上，否則等於白做。
注意：SELL/COVER（平倉）不受此限制，平倉是降低風險的動作，系統不會攔截。
