...
<template>
  <div class="p-6 space-y-6">
    <h2 class="text-2xl font-bold">總覽</h2>
    ...
    <div class="grid grid-cols-4 gap-4">
      <div class="bg-(--color-bg-card) border border-(--color-border) rounded-xl p-4">
        ...
        <div class="text-xl font-bold mt-1" :class="bot.isOnline ? 'text-(--color-success)' : 'text-(--color-danger)'">
          {{ bot.isOnline ? '運行中' : '離線' }}
        </div>
      ...
      <div class="bg-(--color-bg-card) border border-(--color-border) rounded-xl p-4">
        ...
        <div class="text-xl font-bold mt-1">{{ bot.positions.length }}</div>
        <div class="text-xs mt-1" :class="totalPnl >= 0 ? 'text-(--color-success)' : 'text-(--color-danger)'">
          損益: {{ totalPnl >= 0 ? '+' : '' }}{{ totalPnl.toFixed(2) }} USDT
        </div>
      </div>
      ...
      <div v-for="(price, symbol) in bot.latestPrices" :key="symbol"
        ...
        <div class="text-xl font-bold mt-1">${{ price.toLocaleString() }}</div>
      </div>
    </div>
      ...
      <div class="bg-(--color-bg-card) border border-(--color-border) rounded-xl p-4">
        ...
        <div v-for="d in recentDecisions" :key="d.id" class="py-2 border-b border-(--color-border) last:border-0 text-sm">
          <div class="flex justify-between">
            ...
            <span class="text-(--color-text-secondary)">{{ d.symbol }} &middot; {{ (d.confidence * 100).toFixed(0) }}%</span>
          </div>
